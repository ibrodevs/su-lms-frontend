...
export default function CourseEnrollmentsPanel({ courseId }: CourseEnrollmentsPanelProps) {
  ...
  const studentById = useMemo(() => new Map((studentDirectoryQuery.data?.results ?? []).map((student) => [student.id, student])), [studentDirectoryQuery.data]);
