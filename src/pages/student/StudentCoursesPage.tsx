  ...
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-7">
      ...
      {courses.length ? <>
        ...
        <div aria-label="Фильтр по статусу" className="flex max-w-full gap-2 overflow-x-auto pb-1">{statusOptions.map((option) => <button aria-pressed={status === option.value} className={status === option.value ? "shrink-0 rounded-brand border-2 border-ecto bg-ecto/10 px-4 py-2 text-xs font-black text-ecto-dark" : "shrink-0 rounded-brand border-2 border-line bg-paper px-4 py-2 text-xs font-black text-ash hover:bg-mist"} key={option.value} onClick={() => setStatus(option.value)} type="button">{option.label}</button>)}</div>
