...
export default function StudentNotificationsPage() {
  ...
  const notifications = useMemo(() => mockNotifications.filter((item) => { const isRead = item.read || state.readNotifications.includes(item.id); return !state.deletedNotifications.includes(item.id) && `${item.title} ${item.text}`.toLowerCase().includes(query.toLowerCase()) && (filter === "all" || !isRead); }), [filter, query, state.deletedNotifications, state.readNotifications]);
