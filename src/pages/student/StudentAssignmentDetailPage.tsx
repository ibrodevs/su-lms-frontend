...
export default function StudentAssignmentDetailPage() {
  ...
  return <div className="grid gap-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_0.72fr]">
      ...
      <aside className="grid h-fit gap-3 rounded-brand border-2 border-eel bg-ecto/10 p-5"><h2 className="text-lg font-black text-navy">Что дальше</h2><p className="text-sm leading-6 text-graphite">После отправки работа получает статус «Отправлено». Результат проверки будет отображаться здесь после mock-проверки.</p><Link className="text-sm font-black text-macaw-dark underline decoration-2 underline-offset-4" to={`/student/courses/${assignment.courseId}`}>Открыть курс</Link></aside>
    </div>
  </div>;
}
